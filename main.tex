...
\bigskip
More advanced data augmentation techniques, first introduced by Zoph \textit{et al.}\cite{zoph2019learning}, take into account the bounding boxes and only apply data augmentation operations to the objects they contain or to the background. The original work aims at finding the best combination of data augmentation operators for given task and training data. This combination is sought with reinforcement learning and should lead to the best performances when training a model for image classification or object detection. 

In a data randomization scenario, the data augmentation operators presented in this work can be used to increase the variability of training images and widen the source domain data distribution. However, these peculiar transformations have to be used carefully: geometric operations can lead to unintended results if applied to bounding box content or to background only. For example, if the bounding box is small and its content is translated horizontally, the contained object may disappear if it is translated too much. Likewise, if the background (the whole image fixing the content of bounding boxes) is transformed too much, objects can be unintentionally duplicated without associated bounding box. In both cases, this would produce erroneous training images and lead to incorrect training procedures.

A few images produced by these wrong transformations are reported in Figure~\ref{fig:badaugs}.

% TODO: image for unsuccessful transformations on bounding boxes and background
\begin{figure}[ht]
	\centering
	\subcaptionbox{Original \textit{Sim10k} image}{
		\includegraphics[width=.47\linewidth]{imgs/sim10korig.png}
	...
	\caption{Examples of unsuccessful transformations on bounding boxes and image background}
	\label{fig:badaugs}
\end{figure}
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=.7\linewidth]{imgs/fpn.png}
