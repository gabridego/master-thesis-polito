...
Different network architectures have been proposed for different tasks. The basic model is the \textit{feedforward neural network} or \gls{mlp}, whose schema is reported in Figure~\ref{fig:mlp}. Each node of the directed acyclic graph represents a neuron (from which the name \textit{neural network}): the first network layer is the input layer, with as many neurons as input features, while the last layer is the output layer, with one node for any class the network is designed to predict. The intermediate layers are the hidden layers, in which every neuron computes the value of a linear function of the features in the previous layer, followed by a scalar non-linearity. Each output is connected to all nodes in the next layer. The network learns the weights associated to all connections between neurons of different layers.
%TODO: training and more details
\todo{Add training and implementation details}

\subsection{Convolutional neural networks}
%TODO: section on CNNs
\todo{Add a section on CNNs}

\subsection{Computer vision}
%TODO: traditional CV methods
\todo{Mention traditional CV methods}
\glspl{cnn} are a special class of feedworward neural networks, designed to work with inputs that are organized as grids. Hidden units are organized into grids as well. By stacking convolutional layers with non-linear activation functions, increasingly complex non-linear relationships between adjacent pixels in input images are extracted.
...
\begin{figure}[ht]
	...
	\subcaptionbox{CityScapes\cite{cordts2016cityscapes}}{
		\includegraphics[width=.7\linewidth]{imgs/cityscapes.png}
	}
	\subcaptionbox{\acrshort{kitti}\cite{Geiger2013IJRR}}{
		\includegraphics[width=.7\linewidth]{imgs/kitti.png}
	}
	\subcaptionbox{Sim10k\cite{johnsonroberson2017driving}}{
		\includegraphics[width=.7\linewidth]{imgs/sim10k.jpg}
	}
...
\begin{table}[tb]
	...
	\begin{tabularx}{\linewidth}{|X|c|c|c|c|c|c|c|}
		...
		\hline\hline
		Baseline & \checkmark & & & & & & 30.4   \\
		\hline
		\multirow{3}{*}{Image statistics matching} & \checkmark & & & & & & 48.2   \\
		\cline{2-7}
		 & \checkmark & \checkmark & \checkmark & & & & 52.0   \\
		\cline{2-7}
		 & \checkmark & \checkmark & \checkmark & \checkmark & & & \textbf{62.7}   \\
		\hline
		\multirow{3}{*}{DA RetinaNet} & \checkmark & & & & & & 52.8   \\
		\cline{2-7}
...
\begin{table}[tb]
	...
	\begin{tabularx}{\linewidth}{|X|c|c|c|c|c|c|c|c|}
		...
		\hline\hline
		\multirow{2}{*}{Baseline} & & \checkmark & & & & & & 33.7\cite{abramov2020simple}   \\
		\cline{2-8}
		 & \checkmark & \checkmark & & & & & & 46.3   \\
		\hline
		\multirow{3}{=}{Image statistics matching} & \checkmark & \checkmark & & & & & & 55.0   \\
		\cline{2-8}
		...
		\hline
	\end{tabularx}
	\caption{\gls{map} with different domain adaptation method for a Faster \gls{rcnn} model, trained on \textit{Sim10k} and evaluated on \textit{CityScapes}. \gls{fpn} column state if \gls{fpn} is used or not (\texttt{ML} means different domain discriminators are applied to the three ResNet layers used to build the \gls{fpn}). The other columns indicate which transformations are applied to each experiment.}
